 ...
 * extract the front matter from each file into a singular data object
 * then process this data object and create a json5 file inside the strings directory
 * for each piece of front matter found, making sure to put them in the correct locations
 */
...
interface VaultLocationData extends LocationData {
    publish?: boolean;
}

type VaultData = VaultRoleData | VaultCharData | VaultScenarioData | VaultLocationData;
 ...
 */
async function getFrontMatter(path: string): Promise<VaultData | undefined> {
    try {
        const contents: FrontMatterResult<VaultData> = fm(
            await fs.readFile(path, { encoding: 'utf8' }),
        );
        if (
            Object.hasOwn(contents.attributes, 'publish') &&
            contents.attributes.publish === false
        ) {
            return undefined;
...
async function convertFrontMatterToData(filePaths: string[]): Promise<StringData> {
    const frontMatterData: StringData = { chars: [], roles: [], scenarios: [], locations: [] };
    for (let i = 0; i < filePaths.length; i++) {
        ...
        const isLocation = filePaths[i].toLowerCase().includes('location') === true;

        // skip the path if isn't going to be turned into a string
        switch (true) {
            case isChar:
            case isRole:
            case isScenario:
        ...
        switch (true) {
            case isRole:
                frontMatterData.roles.push(data as VaultRoleData);
                break;
            case isChar:
                frontMatterData.chars.push(data as VaultCharData);
                break;
            case isScenario:
                frontMatterData.scenarios.push(data as VaultScenarioData);
                break;
            case isLocation:
                frontMatterData.locations.push(data as VaultLocationData);
                break;
...
async function convertDataToStrings(dataObj: StringData, basePath: string): Promise<void> {
    const create = async (type: 'chars' | 'roles' | 'scenarios' | 'locations') => {
