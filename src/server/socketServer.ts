...
import { Phase } from '../client/stores/storeTypes.ts';
...
export default function swtcSocketServer(
    ...
    function onJoin(
        ...
        if (nameTaken && !playerIsDisconnecting) {
            ...
            if (player === undefined)
                return callback({ status: 'error', error: 'could not find player' });
        ...
        if (narrator) {
            const newChatId = `${playerName.toLowerCase()}_${narrator.name.toLowerCase()}`;
    ...
    function onDisconnect(
        ...
        details: { message: string; description: string; context: string },
    ) {
