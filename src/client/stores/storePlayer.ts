...
import randomise from '../helpers/randomiser/randomiser.ts';
import { socket } from '../helpers/socket.ts';
import { StateCreator } from 'zustand';
import { CombinedSlice, PlayerSlice } from './storeTypes.ts';

export const createPlayerSlice: StateCreator<CombinedSlice, [], [], PlayerSlice> = (set, get) => ({
    players: [],

    setPlayers: (newPlayers) => set(() => ({ players: newPlayers })),

    changePlayerAttribute: (targetId, targetProperty, targetValue, fromServer = false) =>
        set((state) => {
            if (
                ['rRole', 'rChar', 'rState', 'rTeam', 'rVotePower'].includes(targetProperty) &&
                fromServer === false &&
                state.session.sync === true
            ) {
                socket.emit('attribute', {
                    targetId: targetId,
            ...
            if (fromServer === false) {
                ...
                if (!foundPlayer)
                    throw new Error('local player not found while saving session locally');
        ...
        set((state) => {
            ...
            const newPlayers = state.players.map((player) => {
                if (placeReminder && player.id === playerId) {
                    ...
                    if (player.reminders.some((aReminderId) => aReminderId === reminderId))
                        return player;
                    // don't add reminder if the player has the max reminders
                    if (player.reminders.length >= maxReminders) return player;

                    const reminder = GameData.reminders.find(
                        (reminder) => reminder.id === reminderId,
                    );
                    if (!reminder)
                        throw new Error('could not find reminder for associated drop element');
    ...
    randomisePlayers: (chars, roles) =>
        set((state) => {
            ...
            if (state.phase.cycle === 'Night') {
                ...
                newPlayers = NightOrders.addOrderIndicators(
                    ordering,
                    newPlayers,
                    state.purgedOrders,
                );
    ...
    addPlayerNightIndicators: (cycle, chars, roles, purgedOrders, ordering) =>
        set((state) => {
            let newPlayers = state.players;
            ...
            if (newCycle === 'Night') {
                ordering = ordering
                    ? ordering
                    : NightOrders.calculateOrder(newPlayers, chars, roles);
