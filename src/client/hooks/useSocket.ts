    ...
    useEffect(() => {
        const resumeCallback = (error: Error, res: SocketCallbackResponse) => {
            ...
            if (res.status === 'error') {
                ...
                else if (res.error === 'name not in session')
            ...
            if (res.status === 'ok') {
                ...
                socket
                    ...
                    .emit(
                        ...
                        lastSession.playerName,
                        lastSession.playerPronouns,
                        joinCallback,
                    );
            }
        };
        ...
        function onConnect() {
            ...
            socket
                .timeout(20000)
                .emit('resume', lastSession.sessionId, lastSession.playerName, resumeCallback);
        }
        ...
        function onTimer(data: TimerData) {
            if (data.action === 'set') {
                if (data.duration === undefined)
                    throw new Error('error setting timer, duration is undefined');
            ...
            switch (data.action) {
                ...
                case 'createNewChat':
                    if (data.members && data.members.length >= 2)
                        createNewChat(data.chatId, data.members);
        ...
        function onSync(session: SessionData, userId: string | null | undefined) {
            ...
            if (session.timers !== undefined)
                for (const timerData in session.timers) onTimer(session.timers[timerData]);
