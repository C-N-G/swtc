...
function RegularPlayerDetails({
    player,
    handleViewPlayerClick,
    chars,
    roles,
}: RegularPlayerDetailsProps) {
    ...
    function selectBuilder(playerId: string, type: string, list: string[], value: number) {
        return (
            <Autocomplete
                ...
                onChange={(_, newValue) =>
                    handlePlayerDataChange(playerId, type.toLowerCase(), list.indexOf(newValue))
                }
            />
        );
    }
        ...
        <Grid container alignContent="flex-start" sx={{ m: 1 }} rowSpacing={1}>
            ...
            <Grid item xs={6}>
                <Button
                    variant="outlined"
                    onClick={() => {
                        handleDismissalClick(player.id);
                    }}
                >
                    Start Dismissal
                </Button>
                    ...
                    <FormControlLabel
                        control={
                            <Switch
                                ...
                                onChange={() => setShowTrueState((state) => !state)}
                            />
                        }
                        label="True State"
                    />
            ...
            {player.reminders.map((reminderId) => {
                return (
                    <Chip
                        ...
                        label={
                            GameData.reminders.find((reminder) => reminder.id === reminderId)!
                                .flavour
                        }
                    />
                );
