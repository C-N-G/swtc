...
function PlayerVote({ nominatedPlayer, accusingPlayer, user }: PlayerVoteProps & { user: Player }) {
    ...
    const isVotingDisabled = !locationSettings.allow_dead_player_voting && playerIsDead;
        ...
        <Stack sx={{ flexGrow: 1, m: 4 }} justifyContent="space-between">
            ...
            <Stack direction="row" justifyContent="space-between" alignItems="center">
                ...
                <Paper
                    sx={{
                        background:
                            voteTimer.state === 'stopped' ? 'rgb(200,100,100)' : 'rgb(100,200,100)',
                        p: 1,
                        minWidth: '3rem',
                    }}
                >
...
function NarratorVote({
    ...
    function handleVoteStartClick() {
        ...
        socket
            .timeout(5000)
            .emit('timer', { name: 'voteTimer', action: 'start' }, (error, response) => {
