...
function ScenarioLoadingDialog({
    openDialog,
    setOpenDialog,
    setNewScenario,
}: ScenarioLoadingDialogProps) {
    ...
    function validateJSON(input: string) {
        ...
        if (!isValid || convertedJSON! === undefined)
            return 'error: could not parse json from string';
        ...
        convertedJSON.chars.forEach((char) => {
            const foundChar = GameData.chars.find(
                (ele) => ele.name.toLowerCase() === char.toLowerCase(),
            );
        ...
        convertedJSON.roles.forEach((role) => {
            const foundRole = GameData.roles.find(
                (ele) => ele.name.toLowerCase() === role.toLowerCase(),
            );
        ...
        if (!isValid)
            return `error: ${misMatchedItems.length} items found in roles or characteristics were invalid`;
    ...
    function loadJSON() {
        ...
        else if (validationFeedback !== 'valid json found')
            return setInputError('unknown json validation error');
        ...
        setNewScenario(
            new Scenario(
                ...
                loadedScenario.chars.map(
                    (char) => GameData.chars.find((ele) => ele.name === char)!.id,
                ),
    ...
    return (
        <Dialog open={openDialog === OpenDialog.LoadScenario} onClose={handleClose}>
            ...
            <DialogContent>
                <Typography>
                    Please paste in the JSON data from any previously saved scenario files you want
                    to load into the application and then press load!
                </Typography>
                <TextField
                    margin="dense"
                    size="small"
            ...
            <DialogActions sx={{ justifyContent: 'space-between' }}>
                <Button variant="outlined" onClick={loadJSON}>
                    Load
                </Button>
