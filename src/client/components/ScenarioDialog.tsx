...
function ScenarioDialog({ openDialog, handleClose, chars, roles, location }: ScenarioDialogProps) {
    const fullOrdering = useMemo(
        () => NightOrders.calculateFullOrder(chars, roles),
        [chars, roles],
    );
    ...
    const listMaker = (ele: Char | Role, index: number) => {
        const attributes =
            ele.attributes.length > 0 ? (
                <Typography variant="caption">{`[${ele.attributes.join(', ')}]`}</Typography>
            ) : (
                ''
            );
        const name = (
                ...
                <ListItemText
                    ...
                    secondary={
                        <>
                            ...
                            <br />
                            {ele.setup.length > 0 &&
                                `- Setup: ${ele.setup.map((setup, i) => i + 1 + ': ' + setup[0]).join(' ')}`}
                        </>
                    }
                />
            </ListItem>
