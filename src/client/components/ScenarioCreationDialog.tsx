...
function ScenarioCreationDialog({
    openDialog,
    setOpenDialog,
    newScenario,
    setNewScenario,
    ...
    function handleCheckbox(
        id: number,
        type: 'chars' | 'roles',
        roleType?: 'agent' | 'detrimental' | 'antagonist',
    ) {
    ...
    async function handleSave(destination: 'file' | 'save') {
        ...
        if (!isNewScenario) {
            GameData.scenarios = GameData.scenarios.filter(
        ...
        return GameData[plural]
            .filter((item) => {
                ...
                if (
                    type === 'role' &&
                    roleType !== undefined &&
                    (item as Role).type.toLowerCase() === roleType
                )
                    return true;
                else return false;
            })
            .map((item) => {
                ...
                const checkbox = (
                    <Checkbox
                        ...
                        onChange={() =>
                            handleCheckbox(item.id, plural, roleType ? roleType : undefined)
                        }
                        value={item.id}
                    />
                );
                return (
                    <FormControlLabel
                        sx={{
                            color: newScenario[plural].includes(item.id) ? '#a61c1c' : 'inherit',
                        }}
                        key={item.name}
                        control={checkbox}
                        label={title}
    ...
    function handleClose() {
        setNewScenario(new Scenario('', '', '', [], [], {} as Location));
    ...
    return (
        <Dialog open={openDialog === OpenDialog.CreateScenario} onClose={handleClose}>
                    ...
                    <Grid container spacing={2}>
                            ...
                            <TextField
                                ...
                                onChange={(e) => {
                                    setNewScenario((scenario) => ({
                                        ...scenario,
                                        name: maxChar(e.target.value, scenario.name, 32),
                                    }));
                                }}
                            />
                        </Grid>
                        <Grid item xs={2}>
                            <TextField
                                ...
                                value={playerCount}
                                onChange={(e) => {
                                    handlePlayerCount(e.target.value);
                                }}
                            />
                        </Grid>
                        <Grid item xs={12}>
                            <TextField
                                ...
                                onChange={(e) => {
                                    setNewScenario((scenario) => ({
                                        ...scenario,
                                        flavour: maxChar(e.target.value, scenario.flavour, 200),
                                    }));
                                }}
                            />
                        </Grid>
                    ...
                    <Grid container spacing={2} sx={{ justifyContent: 'center' }}>
                        <Grid item sx={{ textAlign: 'center' }}>
                            <Typography variant="h5">
                                Characteristics ({newScenario.chars.length}/
                        ...
                        <Grid item sx={{ textAlign: 'center' }}>
                            <Typography variant="h5">
                                Agent Roles ({roleNums.agent}/{playerFormula + 6})
                            </Typography>
                            <Divider />
                        ...
                        <Grid item sx={{ textAlign: 'center' }}>
                            <Typography variant="h5">
                                Detrimental Roles ({roleNums.detrimental}/{playerFormula + 2})
                            </Typography>
                            <Divider />
                        ...
                        <Grid item sx={{ textAlign: 'center' }}>
                            <Typography variant="h5">
                                Antagonist Roles ({roleNums.antagonist}/1)
                            </Typography>
                            <Divider />
                        ...
                        <Grid item sx={{ textAlign: 'center' }}>
                            ...
                            <Divider />
                            <RadioGroup row defaultValue={GameData.locations[0].id}>
                                {GameData.locations.map((location) => {
                                    const checkbox = (
                                        <Radio
                                            size="small"
                                            checked={newScenario.location.id === location.id}
                                            onChange={() => {
                                                setNewScenario((scenario) => ({
                                                    ...scenario,
                                                    location,
                                    ...
                                    return (
                                        <FormControlLabel
                                            sx={{
                                                color:
                                                    newScenario.location.id === location.id
                                                        ? '#a61c1c'
                                                        : 'inherit',
                                            }}
                                            key={location.name}
            ...
            <DialogActions>
                <Button variant="outlined" disabled={!canSave} onClick={() => handleSave('file')}>
                    Download JSON
                </Button>
                <Button variant="outlined" disabled={!canSave} onClick={() => handleSave('save')}>
                    Save
                </Button>
