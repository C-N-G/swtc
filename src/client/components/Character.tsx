...
function Character(props: CharacterProps) {
    ...
    const getUserTypeCheckedComponent = () => {
        const theUser = props.user === undefined && user !== null ? user : props.user;
...
function PlayerCharacter({ user, useLocal = false }: PlayerCharacterProps) {
    ...
    if (useLocal) {
        // use local player state instead of real state
            ...
            <Grid container justifyContent="left" spacing={2} sx={{ overflow: 'auto' }}>
                    ...
                    <>
                        <Grid item xs={12}>
                            ...
                            <Typography variant="body2">
                                <Box component="span" fontWeight={'Bold'}>
                                    {fullRole['ability'] ? 'Ability: ' : ''}
                                </Box>
                                {fullRole['ability']}
                            </Typography>
                            {fullRole['additional'].map((ele, index) => (
                                <Typography
                                    variant="body2"
                                    key={fullRole['name'] + 'additional' + index}
                                >
                                    {ele}
                                </Typography>
                            ))}
                        ...
                        <Grid item xs={6} textAlign="right">
                            <Typography fontWeight={'Bold'}>
                                {fullRole['attributes'].length > 0 ? 'Attributes' : ''}
                            </Typography>
                        </Grid>
                        ...
                        {fullRole['setup'].length > 0 && (
                            <Grid item xs={12}>
                                <Typography variant="body2" fontWeight={'Bold'}>
                                    {fullRole['setup'].length > 0 ? 'Role Setup' : ''}
                                </Typography>
                                {fullRole['setup'].map((ele, index) => (
                                    <Typography
                                        variant="body2"
                                        key={fullRole['name'] + 'setup' + index}
                                    >
                                        {index + 1}: {ele[0]}
                                    </Typography>
                                ))}
                ...
                {fullChar['name'] !== 'Unknown' && (
                    <Grid item xs={12}>
                        ...
                        <Typography variant="body2">
                            <Box component="span" fontWeight={'Bold'}>
                                {fullChar['ability'] ? 'Ability: ' : ''}
                            </Box>
                            {fullChar['ability']}
                        </Typography>
                        {fullChar['additional'].map((ele, index) => (
                            <Typography variant="body2" key={fullChar['name'] + index}>
                                {ele}
                            </Typography>
                        ))}
                    </Grid>
...
function NarratorCharacter({ user }: NarratorCharacterProps) {
    ...
    const [oldSessionState, setOldSessionState] = useState<OldSessionState>({
        players: [],
        scenarios: [],
    });
    ...
    function syncWithServer() {
        ...
        socket.timeout(5000).emit('sync', syncData, (error, response) => {
            ...
            if (response.status === 'ok') {
                ...
                if (!localPlayer)
                    throw new Error(
                        'error saving local snapshot after syncing, could not find local player',
                    );
    ...
    return (
        <>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center' }}>
                ...
                {sessionSync === false ? (
                    <Button variant="contained" sx={{ ml: 1 }} onClick={handleUndo}>
                        Undo
                    </Button>
                ) : (
                    ''
            ...
            <Button
                variant="contained"
                sx={{ my: 1 }}
                onClick={() => setOpenDialog(OpenDialog.VoteHistory)}
            >
                Vote History
            </Button>
            <Box
            ...
            <VoteHistoryDialog openDialog={openDialog} setOpenDialog={setOpenDialog} />
        </>
    );
}
