...
import { VoteHistoryItem } from '../stores/storeTypes';
...
function VoteHistoryDay({ historyArray, handleOpenClick, openState }: VoteHistoryDayProps) {
            ...
            <Fragment key={index}>
                <ListItem disablePadding>
                    <ListItemButton onClick={() => handleOpenClick(index)}>
                        ...
                        <ListItemText
                            primary={`#${index + 1}. (${item.accuser}) nominated (${item.nominated}) - ${item.voterTotal} vs ${item.abstainerTotal}`}
                        />
                    </ListItemButton>
                </ListItem>
    ...
    const passedThreshold =
        todaysMostVotedPlayer !== null
            ? todaysMostVotedPlayer.votes >= Math.ceil(totalPotentialVoters / 2)
            : false;
    ...
    return (
        <>
            ...
            {hasMostVotedPlayer ? (
                <Typography>
                    Has enough votes for dismissal? {passedThreshold ? `Yes ${threshold}` : 'no'}
                </Typography>
            ) : (
                ''
            )}
            <Typography>
                Players that have Voted: {Array.from(todaysPlayersThatVoted).join(', ')}
            </Typography>
            <List>{todaysVoteHistory}</List>
        </>
    );
}
