...
function NightOrderDialog({ openDialog, handleClose, chars, roles }: NightOrderDialogProps) {
    ...
    const ordering = useMemo(
        () => NightOrders.calculateOrder(players, chars, roles),
        [players, chars, roles],
    );
    ...
    const playerList = ordering.map((nightOrder, index) => {
        ...
        const text = (
            <Typography sx={{ display: 'flex' }}>
                <Typography>{`#${String(displayIndex).padStart(2, '0')}`}</Typography>
                <Typography sx={{ mx: 3 }}>{`${players[nightOrder.playerIndex].name}`}</Typography>
                <AssetIcon asset={attribute} size="s" />
                <Typography sx={{ ml: 0.5 }}>{`${attribute.name}`}</Typography>
            </Typography>
        );
        ...
        return (
            <Fragment key={'orderListKey' + index + nightOrder.name}>
                <ListItem secondaryAction={button} disablePadding>
                    ...
                    <ListItemButton
                        disabled={removed}
                        onClick={() => handleOpenClick(nightOrder.id)}
                    >
                        ...
                        <ListItemText
                            ...
                            sx={
                                removed
                                    ? { textDecoration: 'line-through', color: 'rgb(255, 0, 0)' }
                                    : {}
                            }
                        />
                    </ListItemButton>
                ...
                <Collapse in={openState.has(nightOrder.id)} timeout="auto" unmountOnExit>
                    <Typography component={'span'}>
                        {attrLong} Ability: {attribute.ability}
                    </Typography>
                </Collapse>
            </Fragment>
        );
