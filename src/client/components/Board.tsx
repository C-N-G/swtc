...
function Board() {
    ...
    const [fullChars, fullRoles] = useMemo(
        () => GameData.getFullFilteredValues(scenarios),
        [scenarios],
    );
    ...
    function handleBeginClick() {
        ...
        socket
            ...
            .emit(
                'timer',
                { name: 'voteTimer', action: 'set', duration: timerDuration },
                (error, response) => {
    ...
    function handleVoteFinishClick() {
        ...
        socket
            .timeout(5000)
            .emit('timer', { name: 'voteTimer', action: 'stop' }, (error, response) => {
                if (error) return console.log('Timer Error: server timeout');
                if (response?.error) return console.log('Timer Error:', response.error);
            });
        socket.emit('vote', {
    ...
    const top = drawPlayers
        .slice(0, topNum)
        .map((player, index) => createIndicator(player, index, false));

    const rightside = drawPlayers
        .slice(topNum, topNum + sideNum)
        .map((player, index) => createIndicator(player, index, true));
    ...
    const selectablePlayers = drawPlayers
        .filter((player) => {
            if (!locationSettings.allow_dead_player_nominating && player.state === 0) {
                return false;
            }
            return true;
        })
    ...
    const voteWindow =
        nominatedPlayer && accusingPlayer ? (
            <Vote
    ...
    return (
        ...
        >
            ...
            <Stack direction="row" justifyContent="space-between">
                ...
                <DynamicWindow display={display}>{displayDynamicContent()}</DynamicWindow>
                <Stack width="20%" direction="column" justifyContent="space-evenly">
                    {rightside}
                </Stack>
            </Stack>
