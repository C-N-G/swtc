...
function ScenarioSelectionButton({
    setOpenDialog,
    storeOldData,
    openDialog,
}: ScenarioSelectionButtonProps) {
    ...
    function handleScenarioSelection(scenarioId: string) {
        ...
        if (checked === false) {
            data = [
                ...scenarios,
                GameData.scenarios.find((scenario) => scenario.id === scenarioId)!,
            ];
        }
    ...
    useEffect(() => {
        ...
        const savedScenarios: { [scenarioId: string]: ScenarioData } =
            JSON.parse(savedScenarioJson);
        ...
        Object.keys(savedScenarios).forEach((scenarioId) => {
            ...
            if (charsFound && rolesFound && locationFound) {
                const data = new Scenario(
                    scenarioId,
                    savedScenarios[scenarioId].name,
                    savedScenarios[scenarioId].flavour,
                    savedScenarios[scenarioId].chars.map(
                        (charName) => GameData.chars.find((char) => char.name === charName)!.id,
                    ),
                    savedScenarios[scenarioId].roles.map(
                ...
                console.log(
                    `deleting scenario "${savedScenarios[scenarioId].name}" due to data mismatch`,
                );
    ...
    const allScenarios = GameData.scenarios.map((scenario) => {
        ...
        const listItem = (
            <ListItem key={scenario.name} disablePadding>
                ...
                {isCustom && (
                    <Button
                        ...
                        onClick={() => {
                            setNewScenario(
                                GameData.scenarios.find((ele) => ele.id === scenario.id)!,
                            );
                ...
                {isCustom && (
                    <IconButton aria-label="delete" onClick={() => handleDelete(scenario.id)}>
                        <DeleteIcon />
                    </IconButton>
                )}
            </ListItem>
    ...
    return (
        <>
            <Button
                variant="contained"
                sx={{ my: 1 }}
                onClick={() => setOpenDialog(OpenDialog.Scenario)}
            >
